    ...
    if "ingress" in diff_data:
        ...
        for live_rule in live_ingress:
            if live_rule not in expected_ingress:
                ...
                if confirm_action(f"Revoke unauthorized Inbound rule (Port {from_p}) in {sg_id}", env, is_ssh):
                    try:
...
def remediate_rds_instance(region: str, db_id: str, diff_data: dict, env: str, apply_immediately: bool = False):
    ...
    if updates:
        ...
        if confirm_action(f"Modify RDS {db_id} ({mode_desc}) with configs: {updates}", env, apply_immediately):
            try:
    ...
    for result in drift_results:
        ...
        if result.drift_type.value == "MODIFIED":
            ...
            if result.resource_type == "aws_instance" and "instance_type" in result.diff:
                ...
                remediate_rds_instance(region, result.resource_id, result.diff, env, apply_immediately=False)
